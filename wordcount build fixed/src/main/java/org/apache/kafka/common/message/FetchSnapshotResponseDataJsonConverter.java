...
public class FetchSnapshotResponseDataJsonConverter {
    public static FetchSnapshotResponseData read(JsonNode _node, short _version) {
        ...
        if (_topicsNode == null) {
            ...
            ArrayList<TopicSnapshot> _collection = new ArrayList<TopicSnapshot>();
    ...
    public static class PartitionSnapshotJsonConverter {
        public static PartitionSnapshot read(JsonNode _node, short _version) {
            ...
            JsonNode _bytesNode = _node.get("bytes");
            if (_bytesNode == null) {
                throw new RuntimeException("PartitionSnapshot: unable to locate field 'bytes', which is mandatory in version " + _version);
            } else {
                _object.bytes = ByteBuffer.wrap(MessageUtil.jsonNodeToBinary(_bytesNode, "PartitionSnapshot"));
            }
            return _object;
        }
        public static JsonNode write(PartitionSnapshot _object, short _version, boolean _serializeRecords) {
            ...
            _node.set("bytes", new BinaryNode(MessageUtil.byteBufferToArray(_object.bytes)));
            return _node;
        }
    ...
    public static class TopicSnapshotJsonConverter {
        public static TopicSnapshot read(JsonNode _node, short _version) {
            ...
            if (_partitionsNode == null) {
                ...
                ArrayList<PartitionSnapshot> _collection = new ArrayList<PartitionSnapshot>();
